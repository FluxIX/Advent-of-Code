...
class Room( object ):
   def __init__( self, source ):
      self._parse_input( source )

   def _parse_input( self, source ):
      pattern = r"\s*(?P<name>[a-z]+(?:-+[a-z]+)*)-+(?P<id>\d+)\[(?P<checksum>[a-z]+)\]\s*"

      mo = re.match( pattern, source )
      if mo is not None:
         self._set_encrypted_name( mo.group( "name" ) )
         self._set_sector_id( int( mo.group( "id" ) ) )
         self._set_checksum( mo.group( "checksum" ) )
      else:
   ...
   input = property( fget = get_input )

   def get_encrypted_name( self ):
      return self._encrypted_name

   def _set_encrypted_name( self, value ):
      self._encrypted_name = value

   encrypted_name = property( fget = get_encrypted_name )

   def get_sector_id( self ):
      return self._sector_id

   def _set_sector_id( self, value ):
      self._sector_id = value

   sector_id = property( fget = get_sector_id )
   ...
   checksum = property( fget = get_checksum )

   def get_computed_checksum( self, max_compare_length = 5, source = None ):
      if source is None:
         source = self.get_encrypted_name()

      freq_map = Counter( source )
      del freq_map[ '-' ]
      ...
      for value in freq_map:
         ...
         if freq not in items:
            items[ freq ] = []

         items[ freq ].append( value )

      for key in items:
         items[ key ].sort( reverse = False )

      valid_set = []
      for key in sorted( items.keys(), reverse = True ):
         valid_set.extend( items[ key ] )

      result = "".join( valid_set[ : min( len( freq_map ), max_compare_length ) ] )

      return result

   computed_checksum = property( fget = get_computed_checksum )

   def get_is_valid( self, max_compare_length = 5 ):
      return self.get_checksum() == self.get_computed_checksum( max_compare_length )

   is_valid = property( fget = get_is_valid )

   def __str__( self ):
      return "{}-{:d}[{}]".format( self.get_encrypted_name(), self.get_sector_id(), self.get_checksum() )

   def get_decrypted_name( self ):
      initial_alphabet_value = ord( "a" )
      alphabet_size = 26
      right_shift_quantity = self.get_sector_id() % alphabet_size

      chars = []
      for ch in self.get_encrypted_name():
         if ch == '-':
            new_ch = " "
         else:
            new_ch = chr( ( ( ( ord( ch ) - initial_alphabet_value ) + right_shift_quantity ) % alphabet_size ) + initial_alphabet_value )
   ...
   decrypted_name = property( fget = get_decrypted_name )

def process_rooms( *rooms ):
   rooms = [ Room( r ) for r in rooms ]
   valid_rooms = list( filter( lambda x: x.is_valid, rooms ) )

   print( "Rooms: {:d}, valid rooms: {:d}".format( len( rooms ), len( valid_rooms ) ) )
   print( "Sum of valid room sector IDs: {:d}".format( sum( map( lambda x: x.sector_id, valid_rooms ) ) ) )
   print( "Valid rooms ({:d}): \n{}".format( len( valid_rooms ), "\n".join( list( map( lambda x: x.decrypted_name + " ({:d})".format( x.sector_id ), valid_rooms ) ) ) ) )

ROOMS = list( filter( lambda x: len( x.strip() ) > 0, """
